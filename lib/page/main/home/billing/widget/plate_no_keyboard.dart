...
//        widget.onClose(_plate);
  ...
  Container _wrapButton(FlatButton flatButton) {
    return Container(
      ...
      height: 45,
      decoration: BoxDecoration(
          color: Color(0xffB0B6BF), borderRadius: BorderRadius.circular(8)),
      child: flatButton,
    );
  }
...
//              widget.onClose(_plate);
        ...
        _wrapButton(
          FlatButton(
            ...
            onPressed: () {
              // 车牌号达到七八位数自动关闭键盘
              if (_plate.length == 7 || _plate.length == 8) {
                widget.onClose(_plate);
                Navigator.pop(context);
              } else {
                MyToast.showToast('请输入正确车牌');
              }
            },
            child: Text('确认',
                style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
